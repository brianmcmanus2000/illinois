...
let greatestabsk (a, b, c) k = 
  ...
  mulk (c,c) (fun c_squared ->
    gtk (b_squared,c_squared) (fun b_ge_c ->
      if b_ge_c then
        gtk (a_squared,b_squared) (fun a_ge_b ->
          if a_ge_b then k a else k b)
      else
        gtk (a_squared,c_squared) (fun a_ge_c ->
...
let greatestabsk2 (a, b, c) k = mulk(a,a) (fun a_squared ->
  mulk(b,b) (fun b_squared ->
    mulk(c,c) (fun c_squared ->
      gtk(b_squared,c_squared) (fun passes_check1 ->
        if passes_check1 then gtk(a_squared,b_squared) (fun passes_check_2 ->
          if passes_check_2 then k a else k b)
        else gtk(a_squared,c_squared)(fun passes_check_3 ->
          if passes_check_3 then k a else k c)))));;



greatestabsk2 (-2, 2, 1) string_of_int
(*
