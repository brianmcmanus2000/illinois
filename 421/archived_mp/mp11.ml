...
let rec one_step_exp_cps_eval env exp_cps =
  match exp_cps with
    ...
    | VarCPS(k,s) ->(match lookup_value env s with
      | None -> Failed
      | Some v -> app_cont_to_value env k v)
    | MonOpAppCPS(k,m,s,ec) -> (match lookup_value env s with
      | None -> Failed
      | Some v ->
    ...
    | BinOpAppCPS(k,b,s1,s2,ec) ->(match lookup_value env s1 with
      | None -> Failed
      | Some v1 ->(match lookup_value env s2 with
          | None -> Failed
          | Some v2 ->(match binOpApply b v1 v2 with
            | Value(v') -> app_cont_to_value env k v'
            | Exn(n) -> app_exn_cont_to_value env ec n
          )
        )
      )
    | IfCPS(s,ec1,ec2) ->(match lookup_value env s with
      | None -> Failed
    ...
    | AppCPS(k,f,x,ec) -> (match lookup_value env x with
      | None -> Failed
      | Some v ->
