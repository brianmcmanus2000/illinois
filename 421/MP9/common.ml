...
type mon_op = HdOp | TlOp | PrintOp | IntNegOp | FloatNegOp | FstOp | SndOp
...
let rec string_of_exp = function
 ...
 | BinOpAppExp (b,e1,e2) -> 
   (match b with CommaOp -> ("(" ^ (string_of_exp e1) ^ (string_of_bin_op b) ^
                              (string_of_exp e2) ^ ")")
    | _ -> ((string_of_exp e1) ^ " " ^ (string_of_bin_op b)
 ...
 | RaiseExp e -> "raise " ^ (paren_string_of_exp e)
...
let monop_signature monop = match monop with
    ...
    | FloatNegOp -> ([], mk_fun_ty float_ty float_ty)
